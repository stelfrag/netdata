...
int sql_create_health_log_table(RRDHOST *host) {
    ...
    if (rc != SQLITE_OK) {
        error_report("SQLite error during creation of health log table, rc = %d (%s)", rc, err_msg);
        sqlite3_free(err_msg);
        return 1;
    }
...
void sql_health_alarm_log_cleanup(RRDHOST *host, size_t rotate_every) {
    ...
    sprintf(command, SQL_CLEANUP_HEALTH_LOG(guid,guid,host->health_log_entries_written - rotate_every));

    debug(D_HEALTH, "Executing [%s]", command);
...
void sql_health_alarm_log_count(RRDHOST *host) {
    ...
    sprintf(command, SQL_COUNT_HEALTH_LOG(guid));
    ...
    debug(D_HEALTH, "GREPME COUNT [%d]", sqlite3_column_int(res, 0));
    
    rc = sqlite3_finalize(res);
    if (unlikely(rc != SQLITE_OK))
        error_report("Failed to finalize the prepared statement to count health log entries from db");
    
    debug(D_HEALTH, "GREPME COUNT %s [%ld]", guid, host->health_log_entries_written);
}
...
void sql_health_alarm_log_load(RRDHOST *host) {
    ...
    debug(D_HEALTH, "GREPME Start loading");
    ...
    sprintf(command, SQL_LOAD_HEALTH_LOG(guid,host->health_log.max));
...
void health_alarm_entry_sql2json(BUFFER *wb, uint32_t unique_id, uint32_t alarm_id, RRDHOST *host) {
    ...
    while (sqlite3_step(res) == SQLITE_ROW) {
        ...
        char *edit_command = sqlite3_column_type(res, 18) ? health_edit_command_from_source((char *)sqlite3_column_text(res, 18)) : strdupz("UNKNOWN=0");
...
#define SQL_SELECT_ALL_HEALTH_LOG(guid,limit) "SELECT unique_id, alarm_id FROM health_log_%s order by unique_id desc limit %d;", guid, limit //ADD A LIMIT
