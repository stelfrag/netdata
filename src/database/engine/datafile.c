    ...
    if (!(df->flags & DATAFILE_FLAG_IS_AVAILABLE)) {
        if (reason != DATAFILE_ACQUIRE_RETENTION) {
            ...
            nd_log(
                NDLS_DAEMON,
                NDLP_DEBUG,
                "Accessing datafile %u (tier %d) for the first time -- in %0.2f ms (Superblock %s)",
                df->fileno,
                df->ctx->config.tier,
                (double)(ended_ut - started_ut) / USEC_PER_MS, rc ? "checksum failed, using file anyway" : "ok");
...
//    file_size = ALIGN_BYTES_CEILING(statbuf.st_size);
