...
int rrd_init(char *hostname, struct rrdhost_system_info *system_info, bool unittest) {
    ...
    sql_init_snapshot_database(system_info ? 0 : 1);
    ...
    for(size_t tier = 0; tier < rrdb.storage_tiers ;tier++) {
        netdata_log_info("DEBUG: Scheduling job to check Tier %d snapshot retention", (int) tier);
        metaqueue_build_snapshot(rrdb.multidb_ctx[tier]);
    }
