...
typedef struct ml_event_loop_config {
    ...
    // Command pool (for EXECUTE_BATCH and other control commands)
    ml_training_cmd_pool_t cmd_pool;

    // Dimension inbox — producers write here, event loop drains into pending.
    // Unbounded (JudyL), so dimension queuing never fails.
    Pvoid_t inbox_judy;          // JudyL: metric_id -> (unused)
    SPINLOCK inbox_spinlock;
