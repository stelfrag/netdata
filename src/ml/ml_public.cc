...
void ml_start_threads() {
    ...
    // The event loop thread itself walks all existing dimensions after startup
    // to catch any created before initialization (see ml_training_event_loop).
...
extern "C" size_t ml_get_num_training_threads(void) {
    return Cfg.num_training_threads;
}
